...
- shared domain now exposes cantrip limits, spell-selection modes, labels, max-level derivation, and spell-selection profiles for class and level;
...
2. move `apps/web/src/App.tsx` from its local spell labels, max-level derivation, and spell-cap rules onto the shared domain helper outputs;
